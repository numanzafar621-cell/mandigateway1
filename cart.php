            ...
            <div class="row">
                <div class="col-lg-8">
                    ...
                    foreach ($_SESSION['cart'] as $product_id): 
                        $p = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM products WHERE id = $product_id AND user_id = $user_id"));
                        if ($p):
                            $total += $p['price'];
                    ?>
                        <div class="cart-item d-flex align-items-center">
                            <img src="../<?= htmlspecialchars($p['image']) ?>" width="80" height="80" class="rounded me-3" style="object-fit:cover;">
                            <div class="flex-grow-1">
                                <h6><?= htmlspecialchars($p['title']) ?></h6>
                                <strong class="text-success">Rs. <?= number_format($p['price']) ?></strong>
                            </div>
                            <a href="cart.php?remove=<?= $p['id'] ?>" class="btn btn-sm btn-danger">
                ...
                <div class="col-lg-4">
                    <div class="card sticky-top" style="top:20px;">
                        <div class="card-body">
                            ...
                            <a href="https://wa.me/<?= $store['whatsapp_number'] ?>?text=New%20Order%20%0A%0ATotal%20Amount%3A%20Rs.%20<?= $total ?>%0A%0APlease%20confirm%20my%20order." 
